$(document).ready(function () {
  ...
  const copy_tranx = document.querySelector('#copy-tranx');
  ...
  function copy_tranx_to_clipboard() {
    // Saving the data-trnxid attribute to clipboard.
    // Creating a dummy input field for this process.
    const dummy = document.createElement('input');
    document.body.appendChild(dummy);
    dummy.setAttribute('value', this.dataset.trnxid);
    dummy.style.display = 'block';
    dummy.style.position = 'absolute';
    // Pushing the dummy input field down to hide it from user.
    dummy.style.bottom = '200%';
    dummy.select();
    document.execCommand('copy');
    // Removing the dummy input field after copying to clipboard.
    document.body.removeChild(dummy);
    this.innerText = 'Copied...';
    setTimeout(function(){
      copy_tranx.innerText = 'Copy transaction';
    }, 5000);
  }
